...
use dacite::{
    core::{
        DeviceExtensions, DeviceExtensionsProperties, Extent2D, Instance, InstanceExtensions,
        InstanceExtensionsProperties, PhysicalDevice,
    },
    khr_display::{
        DisplayModePropertiesKhr, DisplayPlaneAlphaFlagBitsKhr, DisplayPlaneAlphaFlagsKhr,
        DisplayPropertiesKhr, DisplaySurfaceCreateFlagsKhr, DisplaySurfaceCreateInfoKhr,
    },
    khr_surface::{SurfaceKhr, SurfaceTransformFlagBitsKhr},
    VulkanObject,
};
...
use super::shell::{init_shell, MyWindowMap, Roles, SurfaceData};
...
fn compute_instance_extensions() -> Result<InstanceExtensions, ()> {
    ...
    let required_extensions = InstanceExtensionsProperties::new()
        .add_khr_display(21)
        .add_khr_surface(25)
        .clone();
...
fn check_device_extensions(physical_device: &PhysicalDevice) -> Result<DeviceExtensions, ()> {
    ...
    let required_extensions = DeviceExtensionsProperties::new()
        .add_khr_display_swapchain(9)
        .add_khr_swapchain(67)
        .clone();
...
struct DisplayData {
    ...
    extent: Rc<Extent2D>,
    modes: Vec<Rc<DisplayModePropertiesKhr>>,
}
...
impl Screen for Display {
    fn get_surface(&self) -> Result<(Rc<SurfaceKhr>, Rc<Extent2D>, Duration), ()> {
        assert_eq!(*self.data.borrow(), None);

        let modes: Vec<Rc<DisplayModePropertiesKhr>> = self
            .properties
            .display
...
pub fn run_udev(
    ...
    let instance_extensions = compute_instance_extensions()?;
