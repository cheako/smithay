...
pub struct VulkanDrawer {
    ...
    pub log: Logger,
    devices: Rc<RefCell<HashMap<vk::PhysicalDevice, Rc<Device>>>>,
}
...
pub struct TextureMetadata {
    pub shm: Option<(
        RefCell<(vk::Buffer, vk::MappedMemoryRange)>,
        Rc<RefCell<BufferData>>,
    )>,
    pub texture: (vk::Image, vk::MappedMemoryRange),
    pub texture_view: vk::ImageView,
    pub fragment: usize,
    pub y_inverted: bool,
    pub format: vk::Format,
    pub uniform: (vk::Buffer, vk::MappedMemoryRange),
    pub uniform_matrix: *mut cgmath::Matrix4<f32>,
    pub descriptor_set: vk::DescriptorSet,
}

impl VulkanDrawer {
    pub fn init(entry: VkEntry, instance: ash::Instance, log: Logger) -> Rc<VulkanDrawer> {
        Rc::new(VulkanDrawer {
            entry,
            instance,
            log,
    ...
    pub fn new_device(&self, device_settings: Rc<DeviceSettings>) -> Result<(), ()> {
        match self.devices.borrow_mut().entry(device_settings.physical_device) {
            Entry::Vacant(o) => {
    ...
    pub fn surface_commit(
        &self,
        buffer: &WlBuffer,
        textures: &mut Option<HashMap<u64, TextureMetadata>>,
    ) -> bool {
        if match textures {
            None => {
...
impl VulkanDrawer {
    ...
    /*
    @@ -294,51 +294,86 @@ impl<F: GLGraphicsBackend + 'static> GliumDrawer<F> {
             compositor_token: MyCompositorToken,
