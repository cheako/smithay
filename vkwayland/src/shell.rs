...
pub struct SurfaceData {
    ...
    pub textures: Option<HashMap<u64, TextureMetadata>>,
    pub dimensions: (i32, i32),
}

fn surface_commit(drawer: &VulkanDrawer, surface: &wl_surface::WlSurface, token: CompositorToken<Roles>) {
    ...
    token.with_surface_data(surface, |attributes| {
        ...
        match attributes.buffer.take() {
            Some(Some((buffer, (_x, _y)))) => {
                ...
                drawer.surface_commit(&data.buffer, &mut data.textures);
            }
            Some(None) => {
                // erase the contents
                let data = attributes.user_data.get_mut::<SurfaceData>().unwrap();
