...
use crate::vulkan_drawer::{TextureMetadata, VulkanDrawer};
...
pub fn init_shell(
    ...
    let (compositor_token, _, _) = compositor_init(
        ...
        move |request, surface, ctoken| match request {
            SurfaceEvent::Commit => surface_commit(&drawer, &surface, ctoken),
...
pub struct SurfaceData {
    ...
    pub textures: Option<RefCell<HashMap<u64, RefCell<TextureMetadata>>>>,
    ...
    token.with_surface_data(surface, |attributes| {
        match attributes.buffer.take() {
            Some(Some((buffer, (_x, _y)))) => {
                ...
                drawer.surface_commit(&buffer, &mut attributes.user_data.textures);
                attributes.user_data.buffer = Some(buffer);
                attributes.user_data.dimensions = (0, 0);
            }
            Some(None) => {
                // erase the contents
                attributes.user_data.buffer = None;
                attributes.user_data.textures = None;
                attributes.user_data.dimensions = (0, 0);
            }
            None => {}
        }
...
fn get_size(attrs: &SurfaceAttributes<SurfaceData>) -> Option<(i32, i32)> {
    Some(attrs.user_data.dimensions)
}
